class audioProcessor extends AudioWorkletProcessor {
  ...
  static getErrorMessage(err, time) {
    ...
    const lineNumber = err.lineNumber
    const columnNumber = err.columnNumber
  ...
  process(inputs, [outputData]) {
    ...
    let { byteSample } = this
    const drawBuffer = []
    const isDiagram = this.drawMode === 'Combined' || this.drawMode === 'Image' || this.drawMode === 'Spectrogram'
    ...
    if (Math.abs(byteSample) > 9007199254740991) {
      this.resetTime()
      return true
    }
  ...
  receiveData(data) {
    ...
    if (data.mode !== undefined) {
      const normalizedMode = audioProcessor.normalizeMode(data.mode)
      this.isFuncbeat = normalizedMode === 'Funcbeat'
  ...
  setFunction(codeText) {
    const chyx = {
      bitC: function (x, y, z) { return x & y ? z : 0 },
      ...
      sinf: function (x) { return Math.sin(x / (128 / Math.PI)) },
      cosf: function (x) { return Math.cos(x / (128 / Math.PI)) },
      tanf: function (x) { return Math.tan(x / (128 / Math.PI)) },
      regG: function (t, X) { return X.test(t.toString(2)) }
    }
    ...
    try {
      if (this.isFuncbeat) {
        ...
        codeText = String(codeText || '').trim().replace(
    ...
    this.errorDisplayed = false
    this.sendData({ error: { message: '', isCompiled: true }, updateUrl: true })
  }
